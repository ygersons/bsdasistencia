        ...
        <div class="card-body">
            <div class="card-body">
                <a class="btn btn-success float-right text-nowra "
                    href="{{ route('admin.reportes.index_asistencia_excel') }}?fecha_inicio={{ $fecha_inicio }}&fecha_fin={{ $fecha_fin }}">
                    <i class="primal"></i> Excel
                </a>

                <div class="float-right">
                    <a class="btn btn-danger float-right text-nowra mr-2"
                        href="{{ route('admin.reportes.asistencia_pdf') }}?fecha_inicio={{ $fecha_inicio }}&fecha_fin={{ $fecha_fin }}"
                        target="_blank">
                        <i class="primal"></i> PDF
                    </a>
                </div>
            </div>
            {!! Form::open(['route' => 'admin.reportes.consultar_asistencia', 'id' => 'formFechas']) !!}
            <div class="container text-left">
                <div class="row align-items-start">
                    <div class="col-3">
                        {!! Form::label('fecha_inicio', 'Fecha Inicio') !!}
                        {!! Form::date('fecha_inicio', isset($fecha_inicio) ? $fecha_inicio : null, [
                            'class' => 'form-control',
                            'id' => 'fecha_inicio',
                        ]) !!}
                        @error('fecha_inicio')
                    ...
                    <div class="col-3">
                        ...
                        {!! Form::date('fecha_fin', isset($fecha_fin) ? $fecha_fin : null, [
                            'class' => 'form-control',
                            'id' => 'fecha_fin',
                        ]) !!}
                        @error('fecha_fin')
                    ...
                    <tbody>
                        @foreach ($asistencia_rpt as $asistencia)
                            <tr>
                    			...
                    			<!--<td>{{ \Carbon\Carbon::parse($asistencia->entradaA)->format('h:i a') }}</td>-->
